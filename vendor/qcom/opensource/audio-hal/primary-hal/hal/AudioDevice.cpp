...
#ifdef SEC_AUDIO_OFFLOAD
                                       fnp_offload_effect_update_output_
#else
    ...
    switch (sources[0].type) {
        case AUDIO_PORT_TYPE_DEVICE: // Patch for audio capture or loopback
            ...
            if (sinks[0].type == AUDIO_PORT_TYPE_MIX) {
                ...
                if (input_source == AUDIO_SOURCE_CAMCORDER && sec_device_->multidevice_rec) {
                    if (bt_sco_on && AudioExtn::audio_devices_cmp(device_types, AUDIO_DEVICE_IN_2MIC)) {
                        device_types.insert(AUDIO_DEVICE_IN_BLUETOOTH_SCO_HEADSET);
                    }
#ifdef SEC_AUDIO_BLE_OFFLOAD
                    else if (AudioExtn::audio_devices_cmp(device_types, AUDIO_DEVICE_IN_BLE_HEADSET)) {
...
create_patch:
    ...
    if (use_feedback_stream) {
        if ((patch_type == AudioPatch::PATCH_PLAYBACK) &&
            (AudioExtn::get_device_types(device_types) == AUDIO_DEVICE_OUT_SPEAKER) &&
...
#ifdef SEC_AUDIO_DSM_AMP
    if (use_feedback_stream) {
        if (feedback.HasFeedbackStreamHandle() &&
            (patch_type == AudioPatch::PATCH_PLAYBACK) &&
            (pal_active_device_count(PAL_DEVICE_OUT_SPEAKER) == 0)) {
...
/*****************************************************************************/
/**                                                                         **/
/**  Samsung Audio HIDL Implementation                                      **/
/**                                                                         **/
/*****************************************************************************/
audio_hw_device_t* AudioDevice::GetAudioDeviceInstance()
{
    return device_.get();
}

extern "C" audio_hw_device_t* sec_get_audio_device_instance()
{
    std::shared_ptr<AudioDevice> adevice = AudioDevice::GetInstance();
#ifdef SEC_AUDIO_AMP_SDHMS
    if (adevice->adev_init_ref_count == 0) {
        AHAL_ERR("no device instance");
        return NULL;
    }
#endif
    return adevice->GetAudioDeviceInstance();
}

extern "C" audio_stream_t* sec_get_audio_stream_instance(audio_hw_device_t *dev, int handle, int type)
{
    std::shared_ptr<AudioDevice> adevice = AudioDevice::GetInstance(dev);
    struct audio_stream *stream = NULL;

    if (type == SEC_DEVICE_STREAM_INPUT) {
        struct stream_in *in;
    ...
    if (stream == NULL) {
        AHAL_ERR("stream for %d(%d) not found", handle, type);
    }
    return stream;
}

void AudioDevice::SecInitAudioStreams(audio_hw_device_t* dev)
{
    std::shared_ptr<AudioDevice> adevice = AudioDevice::GetInstance(dev);

    for (int i = 0; i < SEC_AUDIO_MAX_STREAM_CNT; i++) {
        adevice->sec_out_streams[i] = NULL;
        adevice->sec_in_streams[i] = NULL;
    }
}

// sec_add_audio_stream_l() must be called with adev->lock held
void sec_add_audio_stream_l(struct audio_stream *stream, int type)
{
    std::shared_ptr<AudioDevice> adevice = AudioDevice::GetInstance();
    struct audio_stream **sec_streams = NULL;
    if (type == SEC_DEVICE_STREAM_INPUT) {
        sec_streams = adevice->sec_in_streams;
    } else {
        sec_streams = adevice->sec_out_streams;
    }
    for (int i = 0; i < SEC_AUDIO_MAX_STREAM_CNT; i++) {
        if (*(sec_streams + i) == NULL) {
            *(sec_streams + i) = stream;
            return;
        }
    }
    ALOGE("%s: no space to add the stream %p of type %d", __func__, stream, type);
}

// sec_remove_audio_stream_l() must be called with adev->lock held

void sec_remove_audio_stream_l(struct audio_stream *stream, int type)
{
    std::shared_ptr<AudioDevice> adevice = AudioDevice::GetInstance();
    struct audio_stream **sec_streams = NULL;
    if (type == SEC_DEVICE_STREAM_INPUT) {
    ...
    for (int i = 0; i < SEC_AUDIO_MAX_STREAM_CNT; i++) {
        if (*(sec_streams + i) == stream) {
            *(sec_streams + i) = NULL;
            for (int j = i + 1; j < SEC_AUDIO_MAX_STREAM_CNT; j++) {
                *(sec_streams + j - 1) = *(sec_streams + j);
                if (*(sec_streams + j) == NULL) {
                    break;
                } else if (j == SEC_AUDIO_MAX_STREAM_CNT - 1) {  // End of array
                    *(sec_streams + j) = NULL;
                }
            }
            return;
    ...
    AHAL_ERR("this stream %p of type %d is not in the sec streams", stream, type);
}
#endif  // SEC_AUDIO_HIDL
...
#ifdef SEC_AUDIO_SUPPORT_USB_OFFLOAD
    if (is_usb_dev && (!adevice->USBConnected())) {
        is_usb_dev = false;
        devices = AUDIO_DEVICE_OUT_SPEAKER;
        AHAL_DBG("ignore set device to non existing USB card, use output device(%#x)", devices);
...
#endif
#ifdef SEC_AUDIO_HIDL
    sec_add_audio_stream_l((audio_stream_t*)*stream_out, SEC_DEVICE_STREAM_OUTPUT);
#endif
...
#ifdef SEC_AUDIO_HIDL
    sec_remove_audio_stream_l((audio_stream_t*)stream, SEC_DEVICE_STREAM_OUTPUT);
#endif
...
#ifdef SEC_AUDIO_HIDL
    sec_remove_audio_stream_l((audio_stream_t*)stream, SEC_DEVICE_STREAM_INPUT);
#endif
...
#ifdef SEC_AUDIO_HIDL
    sec_add_audio_stream_l((audio_stream_t*)*stream_in, SEC_DEVICE_STREAM_INPUT);
#endif
...
int AudioDevice::Init(hw_device_t **device, const hw_module_t *module) {
...
#ifdef SEC_AUDIO_SPEAKER_CALIBRATION
    // To write speaker calibration data from efs to sysfs before audio_route_init
    struct str_parms *amp_parms = str_parms_create();
    if (amp_parms) {
#ifdef SEC_AUDIO_SPEAKER_CIRRUS_DSM
        str_parms_add_str(amp_parms,
            AUDIO_PARAMETER_SUBKEY_FACTORY_CALIBRATION_AMP, AUDIO_PARAMETER_VALUE_CIRRUS);
#endif
        sec_cal_set_result(amp_parms);
        str_parms_destroy(amp_parms);
    }
#endif

#ifdef SEC_AUDIO_DSM_AMP
    feedback.init();
    use_feedback_stream = feedback.enable_status();
#endif
    ...
    *device = &(adev_->device_.get()->common);
    ...
    if (access(OFFLOAD_EFFECT_LIBRARY_PATH, R_OK) == 0) {
        offload_effects_lib_ = dlopen(OFFLOAD_EFFECT_LIBRARY_PATH,
                                      RTLD_NOW);
        if (offload_effects_lib_ == NULL) {
            AHAL_ERR("DLOPEN failed for %s", OFFLOAD_EFFECT_LIBRARY_PATH);
        } else {
            AHAL_VERBOSE("DEVICE DLOPEN successful for %s", OFFLOAD_EFFECT_LIBRARY_PATH);
            fnp_offload_effect_update_output_ =
                (int (*)(pal_stream_handle_t*, int))dlsym(
                                    offload_effects_lib_,
                                    "effect_update_payload_by_hal");
        }
    }
#else
    // visualizer lib
...
#ifdef SEC_AUDIO_COMMON
    // DEVICE
    sec_device_ = SecDeviceInit();
    // FACTORY
    factory_ = FactoryInit();
    // EFFECT
    effect_ = EffectInit();
#endif
    mute_ = false;
...
#ifdef SEC_AUDIO_HIDL
    SecInitAudioStreams((audio_hw_device_t*)device_.get());
#endif
...
#ifdef SEC_AUDIO_COMMON
// DEVICE
std::shared_ptr<SecAudioDevice> AudioDevice::SecDeviceInit() {
    std::shared_ptr<SecAudioDevice> sec_device (new SecAudioDevice());

    return sec_device;
}
// FACTORY
std::shared_ptr<AudioFactory> AudioDevice::FactoryInit() {
    std::shared_ptr<AudioFactory> factory (new AudioFactory());

    return factory;
}
// EFFECT
std::shared_ptr<AudioEffect> AudioDevice::EffectInit() {
    std::shared_ptr<AudioEffect> effect (new AudioEffect());
    return effect;
}
#endif
...
int AudioDevice::SetParameters(const char *kvpairs) {
...
#ifdef SEC_AUDIO_COMMON
    // DEVICE
    ret = sec_device_->SecSetParameters(kvpairs);
    if (ret)
        AHAL_ERR("Error in SecSetParameters %d", ret);
    // FACTORY
    ret = factory_->FactorySetParameters(kvpairs);
    if (ret)
        AHAL_ERR("Error in FactorySetParameters %d", ret);
    // EFFECT
    ret = effect_->EffectSetParameters(kvpairs);
    if (ret)
        AHAL_ERR("Error in EffectSetParameters %d", ret);
#endif
...
#ifdef SEC_AUDIO_SUPPORT_AURACAST_REMOTE_MIC
            if (audio_is_usb_out_device(device) || audio_is_usb_in_device(device)) {
                const bool wasAvailable = sec_device_->isUsbInputDeviceAvailable();
                sec_device_->setUsbInputDeviceConnected(usb_card_id_, usb_dev_num_, usb_input_dev_enabled);
                if (wasAvailable != sec_device_->isUsbInputDeviceAvailable()) {
                    sec_device_->updateArmStream();
                }
            } else if (audio_is_ble_broadcast_device(device)) {
                const bool wasAvailable = sec_device_->isBleBroadcastDeviceAvailable();
                sec_device_->setBleBroadcastDeviceConnected(true);
                if (wasAvailable != sec_device_->isBleBroadcastDeviceAvailable()) {
                    sec_device_->updateArmStream();
                }
            }
#endif
    ...
    if (ret >= 0) {
        ...
        if (audio_is_usb_out_device(device) || audio_is_usb_in_device(device)) {
            ...
            param_device_connection.is_bt_offload_enabled = false;
            AHAL_INFO("bt_offload state is disabled");
        }
#endif
        ...
        if (device) {
            ...
            if (audio_is_usb_out_device(device) || audio_is_usb_in_device(device)) {
                ...
                const bool wasAvailable = sec_device_->isBleBroadcastDeviceAvailable();
                sec_device_->setBleBroadcastDeviceConnected(false);
                if (wasAvailable != sec_device_->isBleBroadcastDeviceAvailable()) {
                    sec_device_->updateArmStream();
                }
            }
...
#ifdef SEC_AUDIO_RECORDALIVE_SUPPORT_MULTIDEVICE_PROVIDEO
            // P211022-08752  : fatal provideo bt -> btmix
            for (int i = 0; i < stream_in_list_.size(); i++) {
                stream_in_list_[i]->GetStreamHandle(&stream_in);
                astream_in = adev_->InGetStream((audio_stream_t*)stream_in);
                if (astream_in && astream_in->source_ == AUDIO_SOURCE_CAMCORDER &&
                    ...
                    std::set<audio_devices_t> device_types;
                    device_types.insert(AUDIO_DEVICE_IN_2MIC);
                    astream_in->RouteStream(device_types, true);
                    AHAL_DBG("a2dp_suspended false, switch multidevice to 2mic");
                    break;
                }
            }
#endif
...
#ifdef SEC_PRODUCT_FEATURE_BLUETOOTH_SUPPORT_A2DP_OFFLOAD
    ret = str_parms_get_str(parms, "a2dp_first_param", value, sizeof(value));
    if (ret >= 0) {
        pal_param_bta2dp_sbm_t param_bt_a2dp_sbm;
        param_bt_a2dp_sbm.firstParam = atoi(value);
        ret = str_parms_get_str(parms, "a2dp_second_param", value, sizeof(value));
        if (ret >= 0) {
            param_bt_a2dp_sbm.secondParam = atoi(value);
            ret = pal_set_param(PAL_PARAM_ID_BT_A2DP_SBM_CONFIG, (void *)&param_bt_a2dp_sbm,
                            sizeof(pal_param_bta2dp_sbm_t));
        }
    }
    ...
    if (ret >= 0) {
        ...
        AHAL_INFO("BT A2DP delay report = %d, command received", param_bt_a2dp_delay_report.delay_report);
...
#ifdef SEC_AUDIO_BLUETOOTH
        /*
        * On QC hal, there is a code that sets the path to sco with the "bt sco on" param before the routing req is delivered.
        * Due to the code, if quickly turns "bt sco" from "on" to "off", mute issue occurs.
        * To prevent mute issue, add code that sends the path routing when "bt sco off".
        */
        sec_device_->need_sco_route_update = false;
        if (param_bt_sco.bt_sco_on != bt_sco_on) {
            sec_device_->need_sco_route_update = true;
        }
    ...
    if (ret >= 0) {
        ...
        if (!ret) {
            ...
            AHAL_INFO("TI-SmartPA: setparam ch=%d, idx=%d, len=%d, get=%d",
                    tiSpkProtParam.hdr[0], tiSpkProtParam.hdr[1], param_len,
                    get ? 1 : 0);
            ...
            if (!get) {
                if (param_len > AUDIO_PARAM_MAX_LEN) {
                    ...
                    if (i == param_len) {
                        ...
                        AHAL_ERR("TI-SmartPA: Unable to extract all params");
                    }
                }
            }
        } else {
            AHAL_INFO("TI-SmartPA: invalid params, kvparis=%s\n", kvpairs);
        }
    }
#endif
exit:
...
#ifdef ENABLE_TAS_SPK_PROT
    ret = str_parms_get_str(query, AUDIO_PARAM_TI_SMARTPA, value, sizeof(value));
    if (ret >= 0) {
        int i;
        int param_len;
        if (!tiSpkProtParam.hdr[AUDIO_PARAM_TI_SMARTPA_GET_IDX]) {
            str_parms_add_str(reply, AUDIO_PARAM_TI_SMARTPA, "failed!!");
            AHAL_ERR("TI-SmartPA: get_without_set");
            goto exit;
        } else {
            AHAL_INFO("TI-SmartPA: get ch=%d, idx=0x%x, len=%d\n",
                    tiSpkProtParam.hdr[0],tiSpkProtParam.hdr[1], tiSpkProtParam.hdr[2]);

            pal_tispk_prot_param_t *pTiSpkProtParam = &tiSpkProtParam;
            ret = pal_get_param(PAL_TISA_PARAM_GEN_GETPARAM,
                    (void**) &pTiSpkProtParam, &size, nullptr);
            if (!ret) {
                param_len = tiSpkProtParam.hdr[AUDIO_PARAM_TI_SMARTPA_LEN_IDX];

                for (i = 0; i < AUDIO_PARAM_HDR_LEN - 1; i++)
                    str_parms_add_int(reply, AUDIO_PARAM_TI_HDR[i], tiSpkProtParam.hdr[i]);

                for (i = 0; i < param_len; i++)
                    str_parms_add_int(reply, AUDIO_PARAM_TI_VIDX[i], tiSpkProtParam.data[i]);
            }
        }
        memset(&tiSpkProtParam, 0, sizeof(tiSpkProtParam));
    }
...
#ifdef SEC_AUDIO_COMMON
    if (sec_device_)
        sec_device_->SecGetParameters(query, reply);
    if (effect_)
        effect_->EffectGetParameters(query, reply);
    if (factory_)
        factory_->FactoryGetParameters(query, reply);
#endif
...
void AudioDevice::FillAndroidDeviceMap() {
    ...
    android_device_map_.insert(std::make_pair(AUDIO_DEVICE_IN_MIC3, PAL_DEVICE_IN_SPEAKER_MIC));
    android_device_map_.insert(std::make_pair(AUDIO_DEVICE_IN_MIC4, PAL_DEVICE_IN_SPEAKER_MIC));
    android_device_map_.insert(std::make_pair(AUDIO_DEVICE_IN_MIC3_MIC4, PAL_DEVICE_IN_SPEAKER_MIC));
    android_device_map_.insert(std::make_pair(AUDIO_DEVICE_IN_MULTI_MIC, PAL_DEVICE_IN_SPEAKER_MIC));
#endif
...
#ifdef SEC_AUDIO_SUPPORT_USB_OFFLOAD
bool AudioDevice::USBConnected(void) {
    char path[32] = {0};
    snprintf(path, sizeof(path), "/proc/asound/card%u/stream0", usb_card_id_);
    return access(path, F_OK) == 0;
}
#endif

#ifdef SEC_AUDIO_COMMON
void AudioDevice::SetForceRouteOutStream(const std::set<audio_devices_t>& new_devices, bool force) {
    out_list_mutex.lock();
    for (int i = 0; i < stream_out_list_.size(); i++) {
        std::shared_ptr<StreamOutPrimary> astream_out = NULL;
        astream_out = stream_out_list_[i];
        if (astream_out) {
            ...
            astream_out->ForceRouteStream(new_devices);
            out_list_mutex.lock();
        }
    }
    out_list_mutex.unlock();
...
void AudioDevice::SetForceRouteInStream(const std::set<audio_devices_t>& new_devices) {
    in_list_mutex.lock();
    for (int i = 0; i < stream_in_list_.size(); i++) {
        std::shared_ptr<StreamInPrimary> astream_in = NULL;
        astream_in = stream_in_list_[i];
        if (astream_in) {
            in_list_mutex.unlock();
            astream_in->ForceRouteStream(new_devices);
            in_list_mutex.lock();
        }
    }
    in_list_mutex.unlock();
...
std::shared_ptr<StreamInPrimary> AudioDevice::GetActiveInStream() {
    std::shared_ptr<StreamInPrimary> astream_in = NULL;
    in_list_mutex.lock();
    for (int i = 0; i < stream_in_list_.size(); i++) {
...
// return ActiveIn by input src
std::shared_ptr<StreamInPrimary> AudioDevice::GetActiveInStreamByInputSource(audio_source_t input_source) {
    std::shared_ptr<StreamInPrimary> astream_in = NULL;
    in_list_mutex.lock();
    for (int i = 0; i < stream_in_list_.size(); i++) {
        if (stream_in_list_[i]->mInitialized
          && (stream_in_list_[i]->GetInputSource() == input_source)) {
            AHAL_VERBOSE("Found existing in stream by input source");
            astream_in = stream_in_list_[i];
            break;
        }
    }
...
std::shared_ptr<StreamOutPrimary> AudioDevice::OutGetActiveStream(pal_stream_type_t pal_stream_type) {
    std::shared_ptr<StreamOutPrimary> astream_out = NULL;
    out_list_mutex.lock();
    for (int i = 0; i < stream_out_list_.size(); i++) {
        if ((stream_out_list_[i]->streamAttributes_.type == pal_stream_type)
            && (stream_out_list_[i]->HasPalStreamHandle())) {
            AHAL_VERBOSE("Found active stream associated with pal_stream_type");
            astream_out = stream_out_list_[i];
            break;
        }
    }
...
std::shared_ptr<StreamOutPrimary> AudioDevice::OutGetStreamByUsecase(int usecase) {
    std::shared_ptr<StreamOutPrimary> astream_out = NULL;
    out_list_mutex.lock();
    for (int i = 0; i < stream_out_list_.size(); i++) {
        if (stream_out_list_[i]->GetUseCase() == usecase) {
            AHAL_VERBOSE("Found existing in stream by usecase");
            astream_out = stream_out_list_[i];
            break;
        }
