...
#if IS_ENABLED(CONFIG_SND_SOC_SAMSUNG_AUDIO)
#define MAX_DEFER_COUNT 15
#define AMP_INIT_SUCCESS 1
#define AMP_INIT_FAIL 0
#define AMP_CHECK_NOT_SUPPORT -1
#endif
...
static struct snd_soc_dai_link_component dummy_compononet = {
	.name = "snd-soc-dummy",
...
static int pineapple_tdm_cirrus_init(struct snd_soc_pcm_runtime *rtd)
{
	int i;
...
#if defined(CONFIG_SND_SOC_TAS25XX)
#if IS_ENABLED(CONFIG_SEC_ABC)
void tas_i2c_fail_log(uint32_t i2caddr)
{
	pr_info("%s(%x)\n", __func__, i2caddr);
#if IS_ENABLED(CONFIG_SEC_FACTORY)
	sec_abc_send_event("MODULE=audio@INFO=spk_amp");
...
void tas_amp_fail_log(int32_t ch, int32_t err)
{
	pr_info("%s(ch : %d , error : %d)\n", __func__, ch, err);
#if IS_ENABLED(CONFIG_SEC_FACTORY)
	sec_abc_send_event("MODULE=audio@INFO=spk_amp_short");
#else
	sec_abc_send_event("MODULE=audio@WARN=spk_amp_short");
...
static int pineapple_tdm_tas_init(struct snd_soc_pcm_runtime *rtd)
{
#if IS_ENABLED(CONFIG_SEC_ABC)
	tas25xx_register_i2c_error_callback(tas_i2c_fail_log);
	tas25xx_register_amp_error_callback(tas_amp_fail_log);
#endif
		return 0;
}
#endif
...
static struct snd_soc_dai_link msm_tdm_dai_links[] = {
	{
		...
		.init = &pineapple_tdm_cirrus_init,
		.ops = &msm_common_be_ops,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		SND_SOC_DAILINK_REG(cirrus_pri_tdm_rx_0),
#elif defined(CONFIG_SND_SOC_TAS25XX)
		.dai_fmt = SND_SOC_DAIFMT_DSP_A | SND_SOC_DAIFMT_CBS_CFS
			| SND_SOC_DAIFMT_IB_NF,
		.init = &pineapple_tdm_tas_init,
		.ops = &msm_common_be_ops,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
		SND_SOC_DAILINK_REG(tas25xx_pri_tdm_rx_0),
#else
		.ops = &msm_common_be_ops,
		.ignore_suspend = 1,
		.ignore_pmdown_time = 1,
	...
	},
	{
		...
		.ops = &msm_common_be_ops,
		.ignore_suspend = 1,
		SND_SOC_DAILINK_REG(tas25xx_pri_tdm_tx_0),
#else
		.ops = &msm_common_be_ops,
		.ignore_suspend = 1,
...
static int msm_rx_tx_codec_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	if(!sub_pcb_conn) {
		...
		case CS35L43_MONO:
		case CS35L45_MONO:
			snd_soc_dapm_add_routes(&rtd_card->dapm, cirrus_mono_routes,
				ARRAY_SIZE(cirrus_mono_routes));
			break;
		case CS35L43_STEREO:
		case CS35L45_STEREO:
			snd_soc_dapm_add_routes(&rtd_card->dapm, cirrus_stereo_routes,
				ARRAY_SIZE(cirrus_stereo_routes));
			break;
		case CS35L43_QUAD:
		case CS35L45_QUAD:
			snd_soc_dapm_add_routes(&rtd_card->dapm, cirrus_quad_routes,
...
static int get_audio_amp_ready(enum amp_id id)
{
	...
	if (ret > 0) /* amp first i2c successful or amp fully initialized for playback */
		return AMP_INIT_SUCCESS;
	else
		return AMP_INIT_FAIL;
#endif
	return AMP_CHECK_NOT_SUPPORT;
}
#endif
