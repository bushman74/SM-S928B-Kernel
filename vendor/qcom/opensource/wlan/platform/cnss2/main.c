...
int psm_mode = 0;
int8_t pwr_mode_for_rftestmode = -1;
				...
				size_t count)
{
	...
	sscanf(buf, "%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx",
			(u8*)&mac_from_macloader[0],
			(u8*)&mac_from_macloader[1],
			(u8*)&mac_from_macloader[2],
			(u8*)&mac_from_macloader[3],
			(u8*)&mac_from_macloader[4],
			(u8*)&mac_from_macloader[5]);
...
static ssize_t store_pm_info(struct kobject *kobj,
				...
				size_t count)
{
	int pm_from_macloader;
	cnss_pr_info("%s enter\n", __func__);
	sscanf(buf, "%d", &pm_from_macloader);
	cnss_pr_info("pm_from_macloader %d\n", pm_from_macloader);

	switch (pm_from_macloader) {
	case 1:
		psm_mode = 0;
		break;
	case 0:
		psm_mode = 1;
		pwr_mode_for_rftestmode = 0; // LPI mode
		break;
	case 2:
		psm_mode = 1;
		pwr_mode_for_rftestmode = -1; // referting to 6G AP Reg Info
		break;
	case 3:
		psm_mode = 1;
		pwr_mode_for_rftestmode = 2; // VLP mode
		break;
	case 4:
		psm_mode = 1;
		pwr_mode_for_rftestmode = 1; // SP mode
		break;
	}
...
int cnss_sysfs_get_pwr_mode_for_rftestmode(void)
{
	return pwr_mode_for_rftestmode;
}
EXPORT_SYMBOL(cnss_sysfs_get_pwr_mode_for_rftestmode);
...
int cnss_get_dump_inprogress(struct device *dev, uint8_t *val)
{
	*val = dump_in_progress;
