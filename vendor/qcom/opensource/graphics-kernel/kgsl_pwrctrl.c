					...
					const char *buf, size_t count)
{
	...
	if ((buf[0] == 'c') && ((buf[1] == '\0') || (buf[1] == '\n'))) {
		device->gpu_exception_count[GPU_HANG] = 0;
		device->gpu_exception_count[GPU_PAGE_FAULT] = 0;
		device->gpu_exception_count[GPU_TIMEOUT] = 0;
		device->gpu_exception_count[GPU_PREEMPTION_FAULT] = 0;
		device->gpu_exception_count[GPU_CXWAIT_TIMEOUT] = 0;
		device->gpu_exception_count[GPU_HFI_ERROR] = 0;
	}
					...
					char *buf)
{
	...
	ret += scnprintf(buf+ret, BUF_SIZE-ret, "\"GHG_Q\":\"%d\",", device->gpu_exception_count[GPU_HANG]);
	ret += scnprintf(buf+ret, BUF_SIZE-ret, "\"GPF_Q\":\"%d\",", device->gpu_exception_count[GPU_PAGE_FAULT]);
	ret += scnprintf(buf+ret, BUF_SIZE-ret, "\"GTO_Q\":\"%d\",", device->gpu_exception_count[GPU_TIMEOUT]);
	ret += scnprintf(buf+ret, BUF_SIZE-ret, "\"PFT_Q\":\"%d\",", device->gpu_exception_count[GPU_PREEMPTION_FAULT]);
	ret += scnprintf(buf+ret, BUF_SIZE-ret, "\"CXT_Q\":\"%d\",", device->gpu_exception_count[GPU_CXWAIT_TIMEOUT]);
	ret += scnprintf(buf+ret, BUF_SIZE-ret, "\"GHE_Q\":\"%d\"", device->gpu_exception_count[GPU_HFI_ERROR]);
...
int kgsl_pwrctrl_enable_cx_gdsc(struct kgsl_device *device)
{
	...
	if (!ret) {
		device->gpu_exception_count[GPU_CXWAIT_TIMEOUT]++;
