...
static int count_hlist_entries(struct hlist_head *head)
{
	struct smq_invoke_ctx *ictx;
	struct hlist_node *n;
	int count = 0;
...
static void fastrpc_cleanup_session(struct fastrpc_file *curr_fl)
{
	struct fastrpc_apps *me = &gfa;
	struct fastrpc_file *fl = NULL;
	struct hlist_node *n;
	unsigned long irq_flags = 0;
	int pending_ctx_count = 0, interrupted_ctx_count = 0;
	bool is_process_found = false;

	pr_info("ABHI_DBG: %s called",__func__);
	if (curr_fl->untrusted_process && (curr_fl->cid == CDSP_DOMAIN_ID)) {
		spin_lock_irqsave(&me->hlock, irq_flags);
		hlist_for_each_entry_safe(fl, n, &me->drivers, hn) {
			if ((fl != curr_fl) && fl->untrusted_process && (fl->tgid == curr_fl->tgid) &&
				(fl->sessionid == curr_fl->sessionid) &&
				(fl->cid == curr_fl->cid)) {
				is_process_found = true;
				break;
			}
	...
	if (is_process_found) {
		...
		pending_ctx_count = count_hlist_entries(&fl->clst.pending);
		interrupted_ctx_count = count_hlist_entries(&fl->clst.interrupted);
		spin_unlock_irqrestore(&fl->hlock, irq_flags);
		if (!pending_ctx_count && !interrupted_ctx_count) {
			pr_info("ABHI_DBG: %s: %s triggred file free for %s tgid: %d",
					__func__, current->comm, fl->name, fl->tgid);
			fl->filp->private_data = NULL;
			debugfs_remove(fl->debugfs_file);
			fastrpc_file_free(fl);
		}
	}
