		...
		uint32_t size)
{
	...
	if (i2c_reg_settings == NULL) {
		CAM_ERR(CAM_SENSOR_UTIL, "Failed in i2c list: %p", i2c_reg_settings);
		return NULL;
	} else if (i2c_reg_settings->list_head.next == NULL) {
		CAM_ERR(CAM_SENSOR_UTIL, "Failed in i2c list_head: %p", i2c_reg_settings->list_head.next);
		return NULL;
	}
...
#if !defined(CONFIG_SEC_GPIO_ENABLED_VREG)
...
#if defined(CONFIG_SEC_E3Q_PROJECT) && defined(CONFIG_SENSOR_RETENTION)
const char* ret_io_label = "TELE5X_IO_LDO_EN";
int ret_io_cnt = 0;
#endif
		...
		struct cam_hw_soc_info *soc_info, int gpio_en)
{
	...
	if (gpio_en) {
		for (i = 0; i < size; i++) {
#if defined(CONFIG_SEC_E3Q_PROJECT) && defined(CONFIG_SENSOR_RETENTION)
			if (!strncmp(gpio_tbl[i].label, ret_io_label, 16)) {
				if (ret_io_cnt != 0){
					CAM_INFO(CAM_SENSOR_UTIL,
						"[RET_DBG] gpio %d request skip for sensor retention",
						gpio_tbl[i].gpio);
					continue;
				}
				ret_io_cnt++;
	...
	*pgpio_num_info = kzalloc(sizeof(struct msm_camera_gpio_num_info),
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			...
			CAM_ERR(CAM_SENSOR_UTIL, "gpio-custom3 invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR_UTIL, "gpio-custom3 %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO3]);
	} else {
		rc = 0;
	}
	...
	if (rc != -EINVAL) {
		if (rc < 0) {
			...
			CAM_ERR(CAM_SENSOR_UTIL, "gpio-custom4 invalid %d", val);
			rc = -EINVAL;
			goto free_gpio_info;
		}
		...
		CAM_DBG(CAM_SENSOR_UTIL, "gpio-custom4 %d",
			gpio_num_info->gpio_num[SENSOR_CUSTOM_GPIO4]);
	} else {
		rc = 0;
	}
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	uint32_t						 seq_min_volt = 0;
	uint32_t						 seq_max_volt = 0;
	uint8_t is_retention_power[SENSOR_SEQ_TYPE_MAX] = { 0, };
	...
	// Find retention powers
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		power_setting = &ctrl->power_down_setting[index];
		if ((power_setting->seq_type < SENSOR_SEQ_TYPE_MAX) &&
			(power_setting->config_val > 0))
			is_retention_power[power_setting->seq_type] = 1;
	}
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		if (!is_retention_power[power_setting->seq_type])
			continue;
		...
		switch (power_setting->seq_type) {
		case SENSOR_MCLK:
			CAM_WARN(CAM_SENSOR_UTIL, "retention power up doesn't support MCLK");
			break;
		case SENSOR_RESET:
		case SENSOR_STANDBY:
		case SENSOR_CUSTOM_GPIO1:
		...
		case SENSOR_CUSTOM_GPIO4:
			...
			if (!ctrl->is_retention_power_up[power_setting->seq_type]) {
				CAM_DBG(CAM_SENSOR_UTIL, "gpio set val %d",
					gpio_num_info->gpio_num
					[power_setting->seq_type]);
				...
				ctrl->is_retention_power_up[power_setting->seq_type] = 1;
				CAM_INFO(CAM_SENSOR_UTIL,
					"[RET_DBG] GPIO %d for retention power up",
		...
		case SENSOR_CUSTOM_REG4:
			...
			if (!ctrl->is_retention_power_up[power_setting->seq_type]) {
				if (power_setting->seq_val < num_vreg) {
					...
					ctrl->is_retention_power_up[power_setting->seq_type] = 1;
					CAM_INFO(CAM_SENSOR_UTIL,
						"[RET_DBG] %s for retention power up",
						soc_info->rgltr_name[vreg_idx]);
				} else {
					CAM_ERR(CAM_SENSOR_UTIL, "usr_idx:%d dts_idx:%d",
						power_setting->seq_val, num_vreg);
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	// Find retention power
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		pd = &ctrl->power_down_setting[index];
		if ((pd->seq_type < SENSOR_SEQ_TYPE_MAX) &&
			(pd->config_val > 0))
			is_retention_power[pd->seq_type] = 1;
	}
	...
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		...
		if (!is_retention_power[pd->seq_type])
			continue;
		...
		case SENSOR_CUSTOM_GPIO4:
			...
			if (ctrl->is_retention_power_up[pd->seq_type]) {
				cam_res_mgr_gpio_set_value(
					gpio_num_info->gpio_num
					[pd->seq_type],
					0);
				ctrl->is_retention_power_up[pd->seq_type] = 0;
				CAM_INFO(CAM_SENSOR_UTIL,
					"[RET_DBG] GPIO %d for retention power down",
			...
			if(ctrl->is_retention_power_up[pd->seq_type]) {
				...
				if (ps) {
					if (pd->seq_val < num_vreg) {
						...
						ctrl->is_retention_power_up[pd->seq_type] = 0;
						CAM_INFO(CAM_SENSOR_UTIL,
							"[RET_DBG] %s for retention power down",
							soc_info->rgltr_name[ps->seq_val]);
					} else {
						CAM_ERR(CAM_SENSOR_UTIL,
							"seq_val:%d > num_vreg: %d",
...
struct st_vc_addr {
	...
	[e_vc_addr_260_264] = { 0x0260, 0x0264, CAMERA_SENSOR_I2C_TYPE_BYTE },
	[e_vc_addr_110_264] = { 0x0110, 0x0264, CAMERA_SENSOR_I2C_TYPE_WORD },
...
struct st_sensordbg_addr {
	...
	uint32_t aeb_pd_vc_l;
	uint32_t aeb_pd_vc_s;
...
inline int32_t get_vc_pick_idx(uint32_t sensor_id)
{
	...
	case SENSOR_ID_IMX854:
		return e_vc_addr_110_30a4;
	}
	return e_vc_addr_invalid;
}


int cam_sensor_i2c_write(struct cam_sensor_ctrl_t *s_ctrl,
		...
		enum camera_sensor_i2c_type data_type)
{
	...
	if (rc < 0) {
		CAM_ERR(CAM_SENSOR_UTIL, "i2c byte write failed addr : 0x%x data : 0x%x", addr, data);
		goto free_reg_setting;
	}

	CAM_DBG(CAM_SENSOR_UTIL, "addr = 0x%x data: 0x%x", addr, data);
...
{
	// When AEB is ON, sensor is HP2 or GN3 (meaning AEB60fpsFLL W/A is needed)
	// if it's detected that L FLL mismatching to S FLL,
	int rc = 0;
	bool is_aeb_wa_required = false;
	...
	is_aeb_wa_required = ((seq_sen_id == e_seq_sensor_idn_s5khp2) ||
		(seq_sen_id == e_seq_sensor_idn_s5kgn3));

	if ((true == cam_sensor_check_aeb_on(s_ctrl)) &&
		(true == is_aeb_wa_required)) {
		...
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", sensordbg_addr_info[seq_sen_id].aeb_fll_l);
		}
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			...
			sensordbg_addr_info[seq_sen_id].data_sz, false);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", sensordbg_addr_info[seq_sen_id].aeb_fll_s);
		}

		// if L FLL differs from S, forcibly set same 60fps FLL to L
		if (val_aeb_fll_l != val_aeb_fll_s) {
			CAM_INFO(CAM_SENSOR_UTIL, "[SEN_DBG] %s recover AEB L fll 0x%x", s_ctrl->sensor_name, val_aeb_fll_s);
			cam_sensor_i2c_write(s_ctrl, 
				sensordbg_addr_info[seq_sen_id].aeb_fll_l,
				val_aeb_fll_s,
				CAMERA_SENSOR_I2C_TYPE_WORD,
				sensordbg_addr_info[seq_sen_id].data_sz
			);
		}
	}
}
	...
	)
{
	...
	if (vc_pick_idx == e_vc_addr_invalid) {
		CAM_DBG(CAM_SENSOR_UTIL, "invalid vc_pick_idx");
		return;
	}
	seq_sen_id = get_seq_sen_id(s_ctrl->sensordata->slave_info.sensor_id);
	...
	if (i2c_list->op_code == CAM_SENSOR_I2C_WRITE_RANDOM) {
		for (i = 0; i < i2c_list->i2c_settings.size; i++) {
			...
			if (need_to_check_exp_ctl_violation) {
				...
				if ((i2c_list->i2c_settings.reg_setting[i].reg_addr == sensordbg_addr_info[seq_sen_id].aeb_pd_vc_l) ||
					(i2c_list->i2c_settings.reg_setting[i].reg_addr == sensordbg_addr_info[seq_sen_id].aeb_pd_vc_s)) {
					*update_type |= e_sensor_upd_event_dual_pd_vc;
				}
			}
		}
	}
...
bool cam_sensor_check_aeb_on(struct cam_sensor_ctrl_t* s_ctrl)
{
	...
	if (!slave_info) {
		CAM_ERR(CAM_SENSOR_UTIL,	" failed: %pK",
			slave_info);
		return false;
	}
	...
	if (rc < 0) {
		CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", reg_addr_aeb_ctrl);
	}
...
void cam_sensor_dbg_print_vc(struct cam_sensor_ctrl_t* s_ctrl)
{
	...
	if (is_aeb_activated)
	{
		...
		if (sensordbg_addr_info[seq_sen_id].aeb_vc_sz != CAMERA_SENSOR_I2C_TYPE_INVALID) {
			...
			if (rc < 0) {
				CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", sensordbg_addr_info[seq_sen_id].aeb_vc_l);
			}
			rc = camera_io_dev_read(
				&(s_ctrl->io_master_info),
				...
				sensordbg_addr_info[seq_sen_id].aeb_vc_sz, false);
			if (rc < 0) {
				CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", sensordbg_addr_info[seq_sen_id].aeb_vc_s);
			}

			CAM_INFO(CAM_SENSOR_UTIL, "[AEB_DBG]%s[%s] AEB switched on : VC img_l(0x%x) 0x%x img_s(0x%x) 0x%x, reqId %lld",
				s_ctrl->is_bubble_packet == true ? "[BUBBLE]" : "",
				s_ctrl->sensor_name,
				sensordbg_addr_info[seq_sen_id].aeb_vc_l, vc_l, 
				sensordbg_addr_info[seq_sen_id].aeb_vc_s, vc_s, s_ctrl->last_applied_req);
		} else {
			CAM_INFO(CAM_SENSOR_UTIL, "[AEB_DBG]%s[%s] AEB switched on : odd vc, reqId %lld",
				s_ctrl->is_bubble_packet == true ? "[BUBBLE]" : "",
				s_ctrl->sensor_name, s_ctrl->last_applied_req);
		}
	} else {
		if (vc_pick_idx == e_vc_addr_invalid) {
			CAM_INFO(CAM_SENSOR_UTIL, "[AEB_DBG]%s[%s] AEB off, reqId %lld",
				s_ctrl->is_bubble_packet == true ? "[BUBBLE]" : "",
				s_ctrl->sensor_name, s_ctrl->last_applied_req);
		} else {
			rc = camera_io_dev_read(
			...
			if (rc < 0) {
				CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", vc_addr_info[vc_pick_idx].img_addr);
			}
			...
			rc = camera_io_dev_read(
				...
				vc_addr_info[vc_pick_idx].data_sz, false);
			if (rc < 0) {
				CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", vc_addr_info[vc_pick_idx].pd_addr);
			}
			...
			if (is_fcm_debugging == true) {
				...
				CAMERA_SENSOR_I2C_TYPE_WORD, false);
				if (rc < 0) {
					CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", fcm_idx_addr);
				}
			}
			...
			CAM_INFO(CAM_SENSOR_UTIL, "[AEB_DBG]%s[%s] AEB off, VC img 0x%x, hpd 0x%x, vpd 0x%x (%dByte) fcm_idx 0x%x, reqId %lld",
				s_ctrl->is_bubble_packet == true ? "[BUBBLE]" : "",
				s_ctrl->sensor_name,
...
void cam_sensor_dbg_print_dual_pd_vc(struct cam_sensor_ctrl_t* s_ctrl)
{
	int	rc = 0;
	uint32_t aeb_pd_vc_l = 0, aeb_pd_vc_s = 0;
	uint32_t seq_sen_id = get_seq_sen_id(s_ctrl->sensordata->slave_info.sensor_id);
	...
	rc = camera_io_dev_read(
		&(s_ctrl->io_master_info),
		sensordbg_addr_info[seq_sen_id].aeb_pd_vc_l,
		&aeb_pd_vc_l, CAMERA_SENSOR_I2C_TYPE_WORD,
		CAMERA_SENSOR_I2C_TYPE_BYTE, false);
	if (rc < 0) {
		CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", sensordbg_addr_info[seq_sen_id].aeb_pd_vc_l);
	}

	rc = camera_io_dev_read(
		&(s_ctrl->io_master_info),
		sensordbg_addr_info[seq_sen_id].aeb_pd_vc_s,
		&aeb_pd_vc_s, CAMERA_SENSOR_I2C_TYPE_WORD,
		CAMERA_SENSOR_I2C_TYPE_BYTE, false);
	if (rc < 0) {
		CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", sensordbg_addr_info[seq_sen_id].aeb_pd_vc_s);
	}

	CAM_INFO(CAM_SENSOR_UTIL, "[AEB_DBG][%s] pdaf vc l 0x%x, s 0x%x, reqId %lld",
		s_ctrl->sensor_name,
		aeb_pd_vc_l, aeb_pd_vc_s, s_ctrl->last_applied_req);
}
...
void cam_sensor_dbg_exp_violation(struct cam_sensor_ctrl_t* s_ctrl)
{
	...
	if (is_aeb_activated) {
		...
		rc = camera_io_dev_read(
			...
			sensordbg_addr_info[seq_sen_id].data_sz, false);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", sensordbg_addr_info[seq_sen_id].aeb_cit_l);
		}
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			sensordbg_addr_info[seq_sen_id].aeb_fll_l,
		...
		rc = camera_io_dev_read(
			...
			sensordbg_addr_info[seq_sen_id].data_sz, false);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", sensordbg_addr_info[seq_sen_id].aeb_cit_s);
		}
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			sensordbg_addr_info[seq_sen_id].aeb_fll_s,
		...
		if (long_cit_violation || short_cit_violation)
			CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] %s %s (l: 0x%x 0x%x, s: 0x%x 0x%x)",
	...
	// 3. if AEB off
		...
		rc = camera_io_dev_read(
			...
			sensordbg_addr_info[seq_sen_id].data_sz, false);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", sensordbg_addr_info[seq_sen_id].cit);
		}
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			...
			sensordbg_addr_info[seq_sen_id].data_sz, false);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR_UTIL, "Failed to read 0x%x", sensordbg_addr_info[seq_sen_id].fll);
		}
		// compare cit and FLL
		cit_violation = (val_cit > val_fll) ? true : false;
		if (cit_violation)
			CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] CIT_VIOLATION (0x%x 0x%x)",
				val_cit, val_fll);
	}
}
...
void cam_sensor_dbg_print_by_upd_type(struct cam_sensor_ctrl_t* s_ctrl, int32_t sen_update_type)
{
	...
	if ((sen_update_type & e_sensor_upd_event_dual_pd_vc) == e_sensor_upd_event_dual_pd_vc)
		cam_sensor_dbg_print_dual_pd_vc(s_ctrl);
}
	...
	int i2c_debug_cnt)
{
	...
	if (strstr(debug_sensor_name, s_ctrl->sensor_name) != NULL) {
		CAM_INFO(CAM_SENSOR_UTIL, "sensor_name:%s sz %d", s_ctrl->sensor_name, size);
		for (i = 0; i < size; i++) {
			if (i2c_debug_cnt == 0) {
				const char* str_i2c_cmd[CAM_SENSOR_I2C_POLL + 1] = {
					"[W]", "[WBur]", "[WSeq]", "[R]", "[RSeq]", "[P]" };

				CAM_INFO(CAM_SENSOR_UTIL, "%s%s addr:0x%4x data:0x%4x",
					str_i2c_cmd[i2c_list->op_code],
...
struct st_sensor_reg_dump_addr {
	uint32_t addr;
	enum camera_sensor_i2c_type	data_sz;
	const char* addr_name;
};

struct st_sensor_reg_dump_addr hp2_dump_addr_arr[] = {
...
struct st_sensor_reg_dump_addr gn3_dump_addr_arr[] = {
...
struct st_sensor_reg_dump_addr s5k3lu_dump_addr_arr[] = {
	{ 0x0e16, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB L fll" },
	{ 0x0e18, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB L img vc" },
	{ 0x0e1a, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB L img dt" },
	{ 0x0e1c, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB L pd vc" },
	{ 0x0e26, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB S fll" },
	{ 0x0e28, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB S img vc" },
	{ 0x0e2a, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB S img dt" },
	{ 0x0e2c, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB S pd vc" },
};

struct st_sensor_reg_dump_addr imx564_dump_addr_arr[] = {
	...
	{ 0x0E20, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB long CIT" },
	{ 0x0E2E, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB long FLL" },
	{ 0xE50, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB short CIT" },
	{ 0x0E5E, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB short FLL" },
};


struct st_sensor_reg_dump_addr imx754_dump_addr_arr[] = {
	{ 0x0005, CAMERA_SENSOR_I2C_TYPE_BYTE, "FRAME COUNTER" },
	{ 0x0100, CAMERA_SENSOR_I2C_TYPE_WORD, "STREAM ON" },
	{ 0x0342, CAMERA_SENSOR_I2C_TYPE_WORD, "LINE LENGTH PCK" },
	{ 0x0114, CAMERA_SENSOR_I2C_TYPE_WORD, "CSI_LANE_MODE" },
	...
	{ 0x0E20, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB long CIT" },
	{ 0x0E2E, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB long FLL" },
	{ 0xE40, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB short CIT" },
	{ 0x0E4E, CAMERA_SENSOR_I2C_TYPE_WORD, "AEB short FLL" },
};

const uint32_t dump_addr_page1003[] = { 0x9262 };


void cam_sensor_dbg_regdump_imx564(struct cam_sensor_ctrl_t* s_ctrl)
{
	uint32_t i;
	...
	for (i = 0; i < sizeof(imx564_dump_addr_arr) / sizeof(struct st_sensor_reg_dump_addr); i++)
	{
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			imx564_dump_addr_arr[i].addr,
			&val, CAMERA_SENSOR_I2C_TYPE_WORD,
			imx564_dump_addr_arr[i].data_sz, false);

		if (rc < 0)
			CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] read fail");
		else {
			CAM_INFO(CAM_SENSOR_UTIL, "[SEN_DBG] addr:0x%4x  val:0x%4x //%s(%dB)",
				imx564_dump_addr_arr[i].addr, val,
				imx564_dump_addr_arr[i].addr_name,
				imx564_dump_addr_arr[i].data_sz);
		}
	}
}

void cam_sensor_dbg_regdump_imx754(struct cam_sensor_ctrl_t* s_ctrl)
{
	uint32_t i;
	...
	for (i = 0; i < sizeof(imx754_dump_addr_arr) / sizeof(struct st_sensor_reg_dump_addr); i++)
	{
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			imx754_dump_addr_arr[i].addr,
			&val, CAMERA_SENSOR_I2C_TYPE_WORD,
			imx754_dump_addr_arr[i].data_sz, false);

		if (rc < 0)
			CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] read fail");
		else {
			CAM_INFO(CAM_SENSOR_UTIL, "[SEN_DBG] addr:0x%4x  val:0x%4x //%s(%dB)",
				imx754_dump_addr_arr[i].addr, val,
				imx754_dump_addr_arr[i].addr_name,
				imx754_dump_addr_arr[i].data_sz);
		}
	}
}

void cam_sensor_dbg_regdump_s5k3lu(struct cam_sensor_ctrl_t* s_ctrl)
{
	uint32_t i;
	...
	for (i = 0; i < sizeof(s5k3lu_dump_addr_arr) / sizeof(struct st_sensor_reg_dump_addr); i++)
	{
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			s5k3lu_dump_addr_arr[i].addr,
			&val, CAMERA_SENSOR_I2C_TYPE_WORD,
			s5k3lu_dump_addr_arr[i].data_sz, false);

		if (rc < 0)
			CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] read fail");
		else {
			CAM_INFO(CAM_SENSOR_UTIL, "[SEN_DBG] addr:0x%4x  val:0x%4x //%s(%dB)",
				s5k3lu_dump_addr_arr[i].addr, val,
				s5k3lu_dump_addr_arr[i].addr_name,
				s5k3lu_dump_addr_arr[i].data_sz);
		}
	}
}
...
void cam_sensor_dbg_regdump_gn3(struct cam_sensor_ctrl_t* s_ctrl)
{
	...
	for (i = 0; i < sizeof(gn3_dump_addr_arr) / sizeof(struct st_sensor_reg_dump_addr); i++)
	{
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
...
void cam_sensor_dbg_regdump_hp2(struct cam_sensor_ctrl_t* s_ctrl)
{
	...
	if (rc < 0)
		CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] Failed to write dbg_write_setting_4000 %d", rc);

	for (i = 0; i < sizeof(hp2_dump_addr_arr) / sizeof(struct st_sensor_reg_dump_addr); i++)
	{
		rc = camera_io_dev_read(
			&(s_ctrl->io_master_info),
			hp2_dump_addr_arr[i].addr,
			&val, CAMERA_SENSOR_I2C_TYPE_WORD,
			hp2_dump_addr_arr[i].data_sz, false);

		if (rc < 0)
			CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] read fail");
		else {
			CAM_INFO(CAM_SENSOR_UTIL, "[SEN_DBG] addr: 0x%4x  val: 0x%4x //%s",
				hp2_dump_addr_arr[i].addr, val, hp2_dump_addr_arr[i].addr_name);
		}
	}
	...
	for (i = 0; i < sizeof(dump_addr_page4000) / sizeof(uint32_t); i++)
	{
		...
		if (rc < 0)
			...
			CAM_INFO(CAM_SENSOR_UTIL, "[SEN_DBG] PAGE_4000 addr:0x%4x  val:0x%4x",
	...
	if (rc < 0)
		CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] Failed to write dbg_write_setting_1001 %d", rc);
	...
	for (i = 0; i < sizeof(dump_addr_page1001) / sizeof(uint32_t); i++)
	{
		...
		if (rc < 0)
			...
			CAM_INFO(CAM_SENSOR_UTIL, "[SEN_DBG] PAGE_1001 addr:0x%4x  val:0x%4x",
	...
	if (rc < 0)
		CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] Failed to write dbg_write_setting_1000 %d", rc);
	...
	for (i = 0; i < sizeof(dump_addr_page1000) / sizeof(uint32_t); i++)
	{
		...
		if (rc < 0)
			...
			CAM_INFO(CAM_SENSOR_UTIL, "[SEN_DBG] PAGE_1000 addr:0x%4x  val:0x%4x",
	...
	if (rc < 0)
		CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] Failed to write dbg_write_setting_1003 %d", rc);
	...
	for (i = 0; i < sizeof(dump_addr_page1003) / sizeof(uint32_t); i++)
	{
		...
		if (rc < 0)
			...
			CAM_INFO(CAM_SENSOR_UTIL, "[SEN_DBG] PAGE_1003 addr:0x%4x  val:0x%4x",
...
void cam_sensor_dbg_regdump(struct cam_sensor_ctrl_t* s_ctrl)
{
	...
	case SENSOR_ID_S5KGN3:
		...
		break;
	case SENSOR_ID_IMX564:
		cam_sensor_dbg_regdump_imx564(s_ctrl);
		break;
	case SENSOR_ID_IMX754:
		cam_sensor_dbg_regdump_imx754(s_ctrl);
		break;
	case SENSOR_ID_S5K3LU:
		cam_sensor_dbg_regdump_s5k3lu(s_ctrl);
		break;
	default:
		CAM_ERR(CAM_SENSOR_UTIL, "[SEN_DBG] not supported %d", s_ctrl->sensordata->slave_info.sensor_id);
		break;
	}
}
...
void cam_sensor_dbg_regdump_stream_on_fail_hp2(struct cam_sensor_ctrl_t* s_ctrl)
{
	...
	for (i = 0; i < sizeof(dump_addr_when_stream_on_fail_hp2) / sizeof(uint32_t); i++)
	{
		...
		if (rc < 0)
			...
			CAM_INFO(CAM_SENSOR_UTIL, "[SEN_DBG] addr:0x%4x val:0x%4x",
...
void cam_sensor_dbg_regdump_stream_on_fail(struct cam_sensor_ctrl_t* s_ctrl)
{
	...
	case SENSOR_ID_S5KHP2:
		...
		break;
	default:
		CAM_DBG(CAM_SENSOR_UTIL, "[SEN_DBG] not supported 0x%x", s_ctrl->sensordata->slave_info.sensor_id);
		return;
	}
}
...
int cam_sensor_process_evt_for_sensor_using_i2c(struct cam_req_mgr_link_evt_data* evt_data)
{
	...
	if (!s_ctrl) {
		CAM_ERR(CAM_SENSOR_UTIL, "Device data is NULL");
		return -EINVAL;
	}

	CAM_DBG(CAM_SENSOR_UTIL, "Received evt:%d", evt_data->evt_type);
	...
	case CAM_REQ_MGR_LINK_EVT_ERR:
		CAM_INFO(CAM_SENSOR_UTIL, "[FREEZE_DBG][%s] sof freeze proc_evt %d", s_ctrl->sensor_name,
			evt_data->evt_type);
