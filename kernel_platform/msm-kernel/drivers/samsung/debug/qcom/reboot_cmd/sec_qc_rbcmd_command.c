...
static const struct qc_rbcmd_cmd qc_rbcmd_template[] = {
	...
	SEC_QC_RBCMD_SIMPLE_STRICT("user_dram_test_plus",
				   ...
				   RESTART_REASON_NORMAL),
	SEC_QC_RBCMD_SIMPLE_STRICT("user_dram_test_plus_20min",
				   PON_RESTART_REASON_USER_DRAM_TEST_PLUS_20MIN,
				   RESTART_REASON_NORMAL),
...
static const struct qc_rbcmd_cmd qc_rbcmd_legacy[] = {
	SEC_QC_RBCMD_SIMPLE_STRICT("recovery",
				PON_RESTART_REASON_RECOVERY,
				RESTART_REASON_RECOVERY),
};

static void __rbcmd_apply_legacy(struct qc_rbcmd_cmd *reboot_cmd,
				 ssize_t nr_cmd)
{
	size_t i, j;

	for (i = 0; i < nr_cmd; i++) {
		struct qc_rbcmd_cmd *qrc = &reboot_cmd[i];

		for (j = 0; j < ARRAY_SIZE(qc_rbcmd_legacy); j++) {
			const struct qc_rbcmd_cmd *legacy = &qc_rbcmd_legacy[j];

			if (strcmp(qrc->rc.cmd, legacy->rc.cmd) == 0) {
				memcpy(qrc, legacy, sizeof(*qrc));
				break;
			}
		}
	}
		...
		struct qc_rbcmd_drvdata *drvdata, enum sec_rbcmd_stage s)
{
	...
	if (drvdata->use_legacy)
		__rbcmd_apply_legacy(reboot_cmd, nr_cmd);
