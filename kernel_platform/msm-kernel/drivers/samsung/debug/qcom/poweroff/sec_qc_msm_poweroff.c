...
/*
 * COPYRIGHT(C) 2020-2025 Samsung Electronics Co., Ltd. All Right Reserved.
 */
...
static noinline int
__msm_poweroff_parse_dt_restart_handler_priority(struct builder *bd,
...
static noinline int __msm_poweroff_parse_dt(struct builder *bd)
{
	return sec_director_parse_dt(bd, __msm_poweroff_dt_builder,
				     ARRAY_SIZE(__msm_poweroff_dt_builder));
}

static noinline int __msm_poweroff_probe_prolog(struct builder *bd)
{
...
static noinline void __msm_poweroff_remove_epilog(struct builder *bd)
{
	qcom_set_dload_mode(0);
}

static noinline int __msm_poweroff_get_nv_restart_reason(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
	...
	if (PTR_ERR(nvmem_cell) == -EPROBE_DEFER)
		...
		dev_warn(
			dev,
			"restart_reason nvmem_cell is not ready in the devcie-tree\n");
		return 0;
	}
...
static noinline int __msm_poweroff_get_nv_pon_reason(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
	struct device *dev = bd->dev;
	...
	if (PTR_ERR(nvmem_cell) == -EPROBE_DEFER)
		...
		dev_warn(
			dev,
			"pon_reason nvmem_cell is not ready in the devcie-tree\n");
...
static noinline int
__msm_poweroff_ioremap_qcom_restart_reason(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
...
__msm_poweroff_iounmap_qcom_restart_reason(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
...
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(this, struct qc_msm_poweroff_drvdata, nb_pon_rr);
...
static noinline int __msm_poweroff_register_pon_rr_writer(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
	int err;
...
static noinline void __msm_poweroff_unregister_pon_rr_writer(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);

	sec_qc_rbcmd_unregister_pon_rr_writer(&drvdata->nb_pon_rr);
}
...
static noinline int __msm_poweroff_register_sec_rr_writer(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
	int err;
...
static noinline void __msm_poweroff_unregister_sec_rr_writer(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);

	sec_qc_rbcmd_unregister_sec_rr_writer(&drvdata->nb_sec_rr);
}
...
static noinline int __msm_poweroff_register_reboot_notifier(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
...
static noinline void
__msm_poweroff_unregister_reboot_notifier(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
					  ...
					  unsigned long l, void *d)
{
	...
	__raw_writel(RESTART_REASON_SEC_DEBUG_MODE,
		     drvdata->qcom_restart_reason);
...
static noinline int __msm_poweroff_register_panic_notifier(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
...
static noinline void
__msm_poweroff_unregister_panic_notifier(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
...
static noinline int __msm_poweroff_register_restart_handler(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
...
static noinline void
__msm_poweroff_unregister_restart_handler(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
...
static noinline int __msm_poweroff_probe_epilog(struct builder *bd)
{
	struct qc_msm_poweroff_drvdata *drvdata =
		container_of(bd, struct qc_msm_poweroff_drvdata, bd);
	struct device *dev = bd->dev;
...
static const struct dev_builder __msm_poweroff_dev_builder[] = {
	...
	DEVICE_BUILDER(__msm_poweroff_probe_epilog, NULL),
};
